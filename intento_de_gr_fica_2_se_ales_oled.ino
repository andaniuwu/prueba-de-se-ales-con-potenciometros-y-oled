...
int w[128]; //buffer de la grafica 
int z[128]; //buffer secundario de la grafica 
...
void loop(){
  ...
  w[127]=graficaVoltajee; //asigna el valor escalado a el ultimo dato de la matriz
  ...
  for(int i=127;i>=25;i--){ 
      ...
      display.drawPixel(i, w[i], WHITE); //dibuja punto a punto el contenido de x
      z[i-1]=w[i]; //guarda la informacion desplazada una posicion temporalmente en y
  ...
  for(int i=127;i>=0;i--){
    x[i]=y[i]; //envia los datos desplazados de vuelta a la variable x
       ...
       w[i]=z[i]; //envia los datos desplazados de vuelta a la variable x
  }
}
